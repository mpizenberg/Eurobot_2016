...
void Openfull_bras(void) { // Ouverture à 180° des bras pour fermer les portes.
    if (!PIN_TEAM) {
...
void Catch_bras(void) { // Resserrage des bras contre les blocs de sable.
    if (!PIN_TEAM) {
...
void Close_bras(void) { // Rangement des bras.
    if (!PIN_TEAM) {
...
void Funny_action(void) { // Déploiement du bras pour tendre le 'parasol'.
