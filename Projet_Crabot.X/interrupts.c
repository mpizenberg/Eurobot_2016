...
void __attribute__((interrupt,auto_psv)) _T3Interrupt(void) 
{
    ...
    // pour Èviter les erreurs on veut que la laisse
    ...
    if (Etat_Pin_Laisse) 
    { // Si la laisse est l‡, on maintient le compteur ‡ 30.
        if (Count_Laisse < 30)
            Count_Laisse ++;
    }
    else
    { // S'il n'y a pas la laisse, on dÈcrÈmente le compteur.
        if (Count_Laisse)
            Count_Laisse --;
    }
    ...
    if (Etat_Laisse)
    {
        ...
        {   // Si le compteur atteint 0 (= pas de laisse pendant 30 ms)
            // Alors on active le dÈcompte des 90s et on start.
            Etat_Laisse = 0;
            Active_Delay_90 = 1;
            Delay_90 = 0;
            SendStart();
    ...
    {   // Si on remet la Laisse, le dÈcompte remonte et on peut rÈinit le robot.
        if (Count_Laisse == 30)
        {
            Etat_Laisse = 1;
            Active_Delay_90 = 0;
