...
void set_speed_v(MotionState *state, float v) {
    state->speed.v = v;
}

void set_speed_vt(MotionState *state, float vt) {
    state->speed.vt = vt;
}
...
void set_acceleration_a(MotionState *state, float a) {
    state->acc.a = a;
}

void set_acceleration_at(MotionState *state, float at) {
    state->acc.at = at;
}

void set_acceleration_v_vt(MotionState *state, float v_vt) {
    state->acc.v_vt = v_vt;
}
...
void motion_step(int tics_g, int tics_d, float *commande_g, float *commande_d) {
    ...
    if (!motion_initialized) {
        ...
        *commande_d = 0;
        ...
        //                // D�commenter la ligne suivante si on ne veut pas que le robot reste asservi en position.
        //                //motion_free();
        //                SendDone();
        //            }
        //        }
